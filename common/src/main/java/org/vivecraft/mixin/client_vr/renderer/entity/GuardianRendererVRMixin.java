...
public abstract class GuardianRendererVRMixin {
    ...
    public Vec3 vivecraft$changeEye(GuardianRenderer instance, LivingEntity livingEntity, double d, float f) {
        if (!RenderPassType.isVanilla() && livingEntity == VRState.mc.getCameraEntity()) {
            return ClientDataHolderVR.vrPlayer.getVRDataWorld().getEye(RenderPass.CENTER).getPosition().subtract(0.0D, 0.3D * (double) ClientDataHolderVR.vrPlayer.worldScale, 0.0D);
        }
