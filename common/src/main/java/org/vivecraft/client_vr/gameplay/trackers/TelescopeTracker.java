...
public class TelescopeTracker extends Tracker {
    ...
    private static Vec3 getLensOrigin(int controller) {
        VRData.VRDevicePose vrdata$vrdevicepose = ClientDataHolderVR.vrPlayer.vrdata_room_pre.getController(controller);
    ...
    private static Vec3 getViewVector(int controller) {
        return ClientDataHolderVR.vrPlayer.vrdata_room_pre.getController(controller).getCustomVector(new Vec3(0.0D, -1.0D, 0.0D));
    }
    ...
    public static float viewPercent(int controller) {
        LocalPlayer p = VRState.mc.player;
        if (p != null && ClientDataHolderVR.vrSettings.seated) {
    ...
    private static float viewPercent(int controller, int e) {
        if (e == -1) {
            ...
            VRData.VRDevicePose eye = ClientDataHolderVR.vrPlayer.vrdata_room_pre.getEye(RenderPass.values()[e]);
