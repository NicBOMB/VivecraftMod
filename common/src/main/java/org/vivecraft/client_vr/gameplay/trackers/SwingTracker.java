...
public class SwingTracker extends Tracker
{
    ...
    public boolean isActive()
    {
        ...
        else
        {
            ...
            else if (dh.vrSettings.weaponCollision == VRSettings.WeaponCollision.OFF)
            {
                return false;
            }
            else if (dh.vrSettings.weaponCollision == VRSettings.WeaponCollision.AUTO)
            {
            ...
            else
            {
                ...
                if (dh.vrSettings.vrFreeMoveMode == VRSettings.FreeMove.RUN_IN_PLACE && mc.player.zza > 0.0F)
                {
                    return false;
                }
    ...
    public void doProcess()
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            if (!dh.climbTracker.isGrabbingLadder(i))
            {
                ...
                if (flag3)
                {
                    BlockHitResult blockhitresult = mc.level.clip(new ClipContext(dh.vrPlayer.vrdata_world_pre.hmd.getPosition(), vec3, ClipContext.Block.OUTLINE, ClipContext.Fluid.NONE, mc.player));
                ...
                if (!dh.climbTracker.isClimbeyClimb() || (i != 0 || !VivecraftVRMod.keyClimbeyGrab.isDown(ControllerType.RIGHT)) && flag && (i != 1 || !VivecraftVRMod.keyClimbeyGrab.isDown(ControllerType.LEFT)) && flag)
                {
                    ...
                    BlockHitResult blockhitresult1 = mc.level.clip(new ClipContext(this.lastWeaponEndAir[i], this.miningPoint[i], ClipContext.Block.OUTLINE, ClipContext.Fluid.NONE, mc.player));
    ...
    {
        BlockHitResult blockhitresult = mc.level.clip(new ClipContext(start, end, ClipContext.Block.OUTLINE, ClipContext.Fluid.NONE, mc.player));
